...
# number of pool receptors p is doubled after 3 min

# This program was used to create the figures 6.4(a),(b) of the manuscript.
...
def gillespie_algo(s_i, init, rates, sub_stoch, prod_stoch, tmax, n_max):
    ...
    while (current_time < tmax) and (n_counter < n_max-1):
        ...
        if p_doubled == 0 and current_time >= 3:
            current_species[-1] = current_species[-1]*2
            p_doubled = 1
...
##########################################################
#plot w_i of each synapse during simulation: Fig. 6.4(a) #
##########################################################
...
plt.show()
f.savefig("poolDoubled_plot_wi.pdf", bbox_inches='tight')

##########################################################
######plot R during simulation: Fig. 6.4(b) ##############
##########################################################
...
plt.title(r'$F=0.9$', fontsize=12)
plt.show()
f.savefig("poolDoubled_plot_R.pdf", bbox_inches='tight')

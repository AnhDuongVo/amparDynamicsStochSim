...
# s_1 and s_3 is doubled after 3 min

# This program was used to create the figures 6.7(a),(b) of the manuscript.
...
def gillespie_algo(s_i, init, rates, sub_stoch, prod_stoch, tmax, n_max):
    ...
    while (current_time < tmax) and (n_counter < n_max-1):
        ...
        if s_i_doubled == 0 and current_time >= 3:
            number_slots = [20,0,60,0]
            for i in range(4):
                current_species[4+i] = current_species[4+i] + number_slots[i]
            s_i_doubled = 1
...
phi = 2.67
F = 0.5 # set F for calculating alpha
beta = 60/43 # set beta
...
while True:
    ...
    wi_data = open("wi_"+str(counter_simulation),"a+")
...
##########################################################
#plot w_i of each synapse during simulation: Fig. 6.7(a) #
##########################################################
...
color_lines = ['blue','green','red','cyan']
labels = [r'$w_1$',r'$w_2$', r'$w_3$',r'$w_4$']
for i in [3,2,1,0]:
...
plt.show()
f.savefig("slotsDoubled_plot_wi.pdf", bbox_inches='tight')

##########################################################
#plot R of each synapse during simulation: Fig. 6.7(b) ###
##########################################################
...
plt.title(r'$F=0.5$', fontsize=12)
plt.show()
f.savefig("slotsDoubled_plot_R.pdf", bbox_inches='tight')
